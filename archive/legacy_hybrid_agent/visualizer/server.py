...
from shopper_agent import make_system_prompt, call_shopper_llm, coarse_category
...
class VisualizerHTTPHandler(SimpleHTTPRequestHandler):
    ...
    def handle_sessions_list(self):
        ...
        for s in samples:
            try:
                ...
                if target not in GLOBAL_PRODUCTS:
                    print(f"[Server Warning] Target ASIN {target} not found in catalog. Skipping.")
                    continue
                prod = GLOBAL_PRODUCTS[target]
                card, _ = materialize_hidden_fields(s, GLOBAL_PRODUCTS)
                ...
                target_meta = {
                    ...
                    "avg_rating": prod.get("average_rating", 0.0),
                ...
                print(f"[Server Warning] Error parsing sample {s.get('sample_id')}: {e}")
                continue
    ...
    def handle_manual_start(self, query):
        ...
        # Send full target metadata
        target_meta = {
            "title": target_product.get("title"),
            "brand": target_product.get("store") or target_product.get("details", {}).get("Manufacturer") or "Unknown",
            "features": target_product.get("features", []),
            "description": target_product.get("description", ""),
            "details": target_product.get("details", {}),
            "avg_rating": target_product.get("average_rating", 0.0),
            "rating_number": target_product.get("rating_number", 0),
            "price": target_product.get("price", 0.0),
            "categories": target_product.get("categories", [])
        }
    ...
    def handle_manual_step(self, query):
        ...
        for i, rec in enumerate(recs_meta):
            recs_data.append({
                ...
                "image_url": GLOBAL_IMAGE_MAPPING.get(rec.get("parent_asin")),
    ...
    def handle_stream(self, query):
        ...
        for turn in range(1, 11):
            ...
            if turn == 1:
                ...
                user_message = call_shopper_llm(prompt, system_prompt, "llama3.1")
...
def run_server(port=8080):
    global GLOBAL_AGENT, GLOBAL_CATALOG_IDS, GLOBAL_CATEGORIES, GLOBAL_PRODUCTS, GLOBAL_IMAGE_MAPPING
    ...
    # Load extracted ASIN-to-image mappings if present
    mapping_path = current_dir.parent / "asin_images.json"
    if mapping_path.exists():
        print(f"[Server] Loading image mappings from {mapping_path}...")
        try:
            with open(mapping_path, "r", encoding="utf-8") as f:
                GLOBAL_IMAGE_MAPPING = json.load(f)
            print(f"[Server] Loaded {len(GLOBAL_IMAGE_MAPPING):,} image URLs successfully.")
        except Exception as e:
            print(f"[Server Warning] Failed to load image mappings: {e}")
