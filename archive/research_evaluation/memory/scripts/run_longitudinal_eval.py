"""Chronological evaluator for leakage-safe, post-scoring gated vector memory."""
...
MAX_TURNS = 10
TOP_K = 10
PAIRED_RESPONSE_PROSE = '{"message":"Paired deterministic replay.","ask_attribute":"other"}'
...
def _memory_description(item: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "last_committed_sequence": getattr(state, "last_committed_sequence", None),
        "update_count": getattr(state, "update_count", None),
        "vector_version": "gated-vector-memory-v2" if vector is not None else None,
        "vector_sha256": (
            hashlib.sha256(vector.tobytes(order="C")).hexdigest()
            if vector is not None else None
        ),
        "vector_changed": getattr(item, "vector_changed", None),
    }


def _update_lineage(
    user: Mapping[str, Any], before_sequence_index: int
) -> list[dict[str, Any]]:
    """Evaluator-owned textual lineage for an aggregate memory vector."""

    lineage: list[dict[str, Any]] = []
    for session in user.get("sessions", ()):
        sequence = int(session.get("sequence_index", -1))
        if sequence >= before_sequence_index:
            continue
        directive = session.get("longitudinal_directive", {})
        labels: list[str] = []
        if isinstance(directive, Mapping):
            for values in directive.values():
                if isinstance(values, Sequence) and not isinstance(values, (str, bytes)):
                    labels.extend(str(value) for value in values)
        for value in session.get("intended_historical_facts", ()):
            labels.append(str(value))
        scope = session.get("fact_scope_annotations", {})
        lineage.append({
            "update_id": f"{user['user_id']}:u{sequence}",
            "sequence_index": sequence,
            "session_id": f"{user['user_id']}_s{sequence}",
            "texts": sorted(set(label for label in labels if label.strip())),
            "scopes": deepcopy(scope) if isinstance(scope, Mapping) else {},
            "contributing_sequences": list(range(sequence + 1)),
        })
    return lineage
...
def _run_session(
    ...
    for turn in range(1, max_turns + 1):
        ...
        try:
            mode = "buying" if runtime_sample["scenario_type"] == "buying" else "browsing"
            response = agent.respond(session_id, user_message, turn, top_k, buyer_mode=mode)
    ...
    committed_items = () if committed is None else (committed,)
    ...
    validation = semantic_disclosure_validation(
        ...
        [str(after_end.get("preference_text", ""))] if after_end.get("preference_text") else [],
    )
    scored.update({
    ...
    lineage = _update_lineage(user, sequence_index) if visible_items else []
    scored["contributing_memory_lineage"] = lineage
    ...
    def visible_matches(labels: Sequence[str]) -> list[str]:
        return [
            ...
            if any(
                semantic_match(str(label), text)
                for label in labels for text in value["texts"]
            )
        ]
...
def _metrics(sessions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        **base,
        ...
        "memory_effect_status": "counterfactual_probe_required",
    }
...
def _run_fixed_message_session(
    ...
    *,
    ...
    for turn, user_message in enumerate(shopper_messages, start=1):
        try:
            mode = "buying" if fixture_session.get("scenario_type", "buying") == "buying" else "browsing"
            raw = agent.respond(session_id, str(user_message), turn, top_k, buyer_mode=mode)
    ...
    before_end = _safe_debug(agent, session_id)
    raw_commit = agent.end_session(session_id)
    committed = () if raw_commit is None else (raw_commit,)
    after_end = _safe_debug(agent, session_id)
...
def _record_agent_llm_calls(agent: Any, tape: list[dict[str, Any]]) -> bool:
    ...
    def recorded(
        ...
        # Ranking-dependent candidate prose is intentionally outside the paired
        # parser contract.  Both arms receive the exact same deterministic stub.
        if not response_json:
            return PAIRED_RESPONSE_PROSE
        response = original(
            prompt,
            system_prompt,
            session_id=session_id,
        ...
        for user in ordered_fixture_users(fixture):
            for fixture_session in user["sessions"]:
                ...
                if condition == "NO_HISTORY":
                    ...
                    if snapshot.states or snapshot.commits:
...
def summarize_probe_comparisons(sessions: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for user_id, values in by_user.items():
        ...
        comparisons[user_id] = {
            ...
            "observed_rr_delta_without_passing_gate": delta if not applied else None,
            "history_curve": [
    ...
    return {
        ...
        "status": "gate_never_passed" if not active else "memory_active",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the gated vector-memory longitudinal evaluation")
